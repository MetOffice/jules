...
IF ( l_soil_evap_irrig_separate .AND. .NOT. l_irrig_dmd ) THEN
  errcode = 101
  CALL ereport(RoutineName, errcode,                                           &
               'l_soil_evap_irrig_separate=T requires l_irrig_dmd=T ')
END IF
...
WRITE(lineBuffer,*) ' l_soil_evap_irrig_separate = ',l_soil_evap_irrig_separate
CALL jules_print('jules_irrig',lineBuffer)
...
TYPE :: my_namelist
  ...
  LOGICAL :: l_soil_evap_irrig_separate
  INTEGER :: irr_crop
  INTEGER :: nirrtile
...
IF (mype == 0) THEN
  ...
  my_nml % l_soil_evap_irrig_separate  = l_soil_evap_irrig_separate
...
IF (mype /= 0) THEN
  ...
  l_soil_evap_irrig_separate  = my_nml % l_soil_evap_irrig_separate
