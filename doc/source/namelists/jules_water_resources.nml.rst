...
.. warning::
   The water resource code in JULES is still in development. A limited parametrisation exists but it should be used with caution while development continues. At present all demands for water can only be met by abstraction from the same gridbox as where the demand is, which can lead to unrealistically high estimates of water shortage where in reality water infrastructure is used to move water between locations. Consult the developers for further information.
...
.. nml:group:: Switches that control which water sectors are considered.
   ...
   .. nml:member:: l_water_environment
      ...
      .. note::
         At present :nml:mem:`l_water_environment` must be FALSE as the parametrisation is not yet complete. In the meanwhile this switch is provided to allow the developers to test code.
   ...
   .. nml:member:: l_water_transfers
      ...
      .. note::
         At present :nml:mem:`l_water_transfers` must be FALSE as the parametrisation is not yet complete. In the meanwhile this switch is provided to allow the developers to test code.
