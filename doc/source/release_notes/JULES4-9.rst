 ...
 * Extensions to the parametrisation of moisture stress on vegetation. There is now (a) the option of a stress function that is piece-wise linear in soil potential (:nml:mem:`JULES_VEGETATION::fsmc_shape` = 1) rather than soil volumetric moisture content (:nml:mem:`JULES_VEGETATION::fsmc_shape` = 0) and (b) the option of specifying pft-dependent soil potentials (:nml:mem:`JULES_VEGETATION::l_use_pft_psi`) at which the plant starts to experience water stress (:nml:mem:`JULES_PFTPARM::psi_open_io`) and where the plant is fully stressed (:nml:mem:`JULES_PFTPARM::psi_close_io`). (#541)
